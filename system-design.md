...
**Entry point:** Direction Document submitted to the pipeline.
...
- (Optional)**Approval gate:** Karolina (Director Revenue & Distribution) validates assumptions, including any fields left blank due to Lighthouse data limitations. Only after `FinancialModel.validated == True` does the Assembly Agent insert the financial slide. Users can upload a manually revised Excel to override auto-population.
- **Failure:** Lighthouse unavailable → flag all market assumptions as unverified and surface for Karolina review.
- **Data limitation:** Lighthouse does not provide ADR or occupancy data. The financial model will have explicit gaps for these fields unless Karolina or the BD team supplies them manually (see ADR-011).
...
|---|---|---|---|
| 1 | File storage — where does the Direction Document get saved and how does it enter the pipeline? (SharePoint / watched folder / direct UI upload) | Theo + Douglas / Broad Vision | Mon 28 Apr |
...
| Week 1 | Phase 1 (Claude Desktop) | Custom MCP server. `/market-research` and `/submit-brief` skills. End-to-end submit flow (Claude Desktop → SharePoint `direction.md`). Test with one historical brief (Marrakech). |
| Week 2 | Phase 2: Outline Agent + Validator + UI | Outline Agent constrained by Chapter Library. Validator. Outline Review UI (chapter tree, approve/flag/edit). End-to-end Direction Document → approved outline. |
